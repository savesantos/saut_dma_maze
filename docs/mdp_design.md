...
| Exploration         | $\epsilon$-greedy, **linear decay** $\epsilon_0 = 1.0 \to \epsilon_\text{min} = 0.05$ over the first $N_\text{anneal} = 0.5\,N_\text{episodes}$ episodes, constant thereafter | Identical schedule for SARSA and Q-Learning so the comparison is apples-to-apples. $\epsilon_0 = 1.0$ gives a true random-walk warm-up (covers the state graph at $T_\text{max} = 10\|S\|$); 50/50 anneal/exploit split is the standard choice and easy to plot. $\epsilon_\text{min} = 0.05 > 0$ is required under stochastic dynamics ($p_s = 0.1$) — zero exploration can leave $(s,a)$ pairs unvisited and bias $Q$. $Q_0(s,a) = 0$ by default: with negative step costs this is already optimistic relative to $Q^*$, so we get the optimism effect for free without an extra knob. Boltzmann / UCB / visit-count GLIE rejected: extra hyperparameters and noisier curves dilute the VI vs SARSA vs Q-Learning comparison in 6 IEEE pages. |
